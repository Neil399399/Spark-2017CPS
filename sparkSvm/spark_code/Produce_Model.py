...
def Test_Model(testingRDD, Model):
    ...
    for x in range(0,1):
        ...
        truePositive = labelsAndPreds.filter(lambda p: p[0] == p[1] and p[0] == 1).count()
        falsePositive = labelsAndPreds.filter(lambda p: p[0] != p[1] and p[0] == 1).count()
        trueNegative = labelsAndPreds.filter(lambda p: p[0] == p[1] and p[0] == 0).count()
        falseNegative = labelsAndPreds.filter(lambda p: p[0] != p[1] and p[0] == 0).count()
